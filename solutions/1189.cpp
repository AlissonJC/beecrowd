 ...
 *   mean of the cells that fall inside the LEFT triangle of the matrix
 *   and print it with exactly one digit after the decimal point.
 *
 *   The two diagonals i == j and i + j == 11 cut the 12 x 12 square into
 *   four triangles. The "left" one is everything strictly LEFT of both
 ...
 *   average (if 'M') of the 30 cells in the left triangle.
 *
 * Approach:
 *   Read the operation, fill the 12 x 12 array, scan every cell, and
 *   accumulate values where (j < i && j < 11 - j). The cell count is the
...
int main() {
    ...
    // Accumulate cells strictly left of both diagonals of the 12 x 12 square.
    // Condition j < i  keeps cells left of the main diagonal (the "\" line);
    // condition j < 11 - i  keeps cells left of the anti-diagonal ("/" line).
    // Together they isolate the left triangle shown in green in the figure.
    double sum = 0.0;
    int count = 0;
    for (int i = 0; i < 12; i++) {
        for (int j = 0; j < 12; j++) {
